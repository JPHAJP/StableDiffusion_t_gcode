...
class Graph:
	...
	def saveAsGcodeFile(self, f, scale=1.0):
		...
		def pathGcode(i, insidePath):
			f.write(f"G{1 if insidePath else 0} X{(scale * self[i].y):.2f} Y{(-scale * self[i].x):.2f}\n")
		...
		if len(possibleStartingNodes) != 0:
			...
			while True:
				...
				if len(self[node].connections) == 0:
					...
					f.write(f"G1 X{(scale * self[node].y):.2f} Y{(-scale * self[node].x):.2f}\n")
				else:
...
def parseArgs(namespace):
	...
	argParser.add_argument("-s", "--scale", type=float, default=1.0, metavar="VALUE",
    help="Factor de escala para las coordenadas NC generadas")
...
def main():
	...
	converter.graph.saveAsGcodeFile(Args.output, Args.scale)
